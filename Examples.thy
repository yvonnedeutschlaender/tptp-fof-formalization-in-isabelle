...
"f5 = Equiv (Pred ''A'' [Var ''x'']) (Pred ''B'' [Var ''x''])"
...
lemma f5_is_satisfiable: "\<exists>pI. eval_formula f5 vI fI pI" 
  unfolding f5_def Equiv_def Imp_def
  by auto

lemma f5_is_not_tautology: "\<exists>pI. \<not> eval_formula f5 vI fI pI"
proof (rule exI)
  let ?pI = "\<lambda> p args. (if p = ''A'' then True else False)"
  show "\<not> eval_formula f5 vI fI ?pI"
    unfolding f5_def Equiv_def Imp_def
    by simp
qed
...
proof (rule exI)
  let ?pI = "undefined"
  show "\<not> eval_formula f6 vI fI ?pI"
    unfolding f6_def
    sorry
qed
...
proof (rule exI)
  let ?pI = "undefined"
  show "\<not> eval_formula f7 vI fI ?pI"
    unfolding f7_def
    sorry
qed
...
proof (rule exI)
  let ?pI = "undefined"
  show "\<not> eval_formula f8 vI fI ?pI"
    unfolding f8_def
    sorry
qed

lemma unsatisfiable_f9: "\<not> eval_formula f9 vI fI pI"
  unfolding f9_def
  by simp

lemma unsatisfiable_f10: "\<not> eval_formula f10 vI fI pI"
  unfolding f10_def
  by simp
